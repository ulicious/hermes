...
from algorithm.methods_routing import process_in_tolerance_branches_high_memory,\
...
from algorithm.methods_export import (attach_infrastructure_countries,
                                      apply_export_conversion,
...
def _finalize_routing_branches(branches, old_branches, local_benchmarks, branch_number, data, configuration,
    ...
    # Export branches are deliberately not compared by cost and not collapsed to
    # the cheapest equivalent state. Every generated route gets its own identity.
    branches = drop_branch_comparison_columns(branches.copy())
    ...
    branches.drop(['minimal_total_costs', 'minimal_commodity', 'benchmark'], axis=1,
                  inplace=True, errors='ignore')
    branches = postprocessing_branches(branches, old_branches)
    return (branches, local_benchmarks, branch_number, final_solution, benchmark,
...
def _prepare_infrastructure_branches_for_routing(infrastructure_branches, complete_infrastructure, configuration, data,
    ...
    # Do not preselect a handful of cheap branches in the export runner.
...
def run_export_algorithm(args):
    ...
    complete_infrastructure = attach_infrastructure_countries(complete_infrastructure, data.get('world'))
    ...
    # Export enumeration has no destination benchmark. Infinite commodity limits
    # keep the unchanged routing cost calculations from pruning valid branches.
    ...
    if branches['current_total_costs'].map(math.isinf).all():
    ...
    complete_infrastructure = get_export_infrastructure_scope(
        data, complete_infrastructure, start_country)
    ...
    # No initial benchmark calculation in this runner.
    benchmark = math.inf
    benchmarks = {commodity: math.inf for commodity in data['commodities']['all_commodities']}
    benchmark_locations = {commodity: None for commodity in data['commodities']['all_commodities']}
    benchmark_info = ([], [], [], [], [])
    data['benchmark_info'] = benchmark_info
    data['_benchmark_removal_reported'] = False
    cumulative_benchmark_costs = []
    initial_benchmark_costs = math.inf
    ...
    conversion_branches = branches.copy()
    ...
    while not branches.empty:  # while loop runs as long as branches to process exist
        ...
        if iteration > 0:
            ...
            branches, branch_number = apply_export_conversion(
                conversion_possible_branches, data, branch_number)
            potential_final_solution = None
        ...
        if not branches.empty:
            ...
            if not out_infrastructure_branches.empty:
                ...
                if no_pipeline_gas_branches:
                    outside_options_no_gas \
                        = process_export_out_tolerance_branches(complete_infrastructure,
            ...
            if routing_results:
                ...
                tracker.event(iteration=iteration, phase='routing', method='run_algorithm',
                              ...
                              after=branch_count(branches), removed=0,
        ...
        # A foreign infrastructure is an export terminal. Store it and never
        # expand it in a later iteration.
        branches, newly_completed = split_export_branches(
            branches, complete_infrastructure, start_country)
        if not newly_completed.empty:
            completed_export_branches.append(newly_completed)
        export_branch_snapshot(branches, configuration['path_results'], location_index,
                               iteration, 'active')
        completed_snapshot = (pd.concat(completed_export_branches, ignore_index=False)
                              if completed_export_branches else pd.DataFrame())
        export_branch_snapshot(completed_snapshot, configuration['path_results'], location_index,
                               iteration, 'completed')
    ...
    # Store all export terminals, not only a cost-minimal solution.
    all_completed = (pd.concat(completed_export_branches, ignore_index=False)
                     if completed_export_branches else pd.DataFrame())
    export_branch_snapshot(all_completed, configuration['path_results'], location_index,
                           iteration, 'all_completed')
    completion_marker = os.path.join(configuration['path_results'],
                                     'export_infrastructure_branches', str(location_index), '_complete')
    with open(completion_marker, 'w', encoding='utf-8') as marker:
        marker.write(str(len(all_completed)))
