...
def group_LineStrings(name, num_cores, path_to_file, path_processed_data, use_minimal_example=False):
    ...
    if use_minimal_example:
        data = data.loc[data['EndRegion'].isin(['Europe', 'Eurasia'])]
    ...
    data_new = gpd.GeoDataFrame(pd.Series(lines).apply(shapely.wkt.loads), columns=['geometry'])#.loc[0:1000]
    data_new.set_geometry('geometry')
    data_new_exploded = data_new.explode(ignore_index=True)

    def divide_data(data, chunk_size=100):

        # Adjust chunk size s.t. multiprocessing is optimally used
        if len(data) / chunk_size < 120:
            chunk_size = max(math.ceil(len(data) / 120), 50)

        divided_data = []
        random.shuffle(data)
        for i in range(0, len(data), chunk_size):
            divided_data.append(data[i:i+chunk_size])
