...
from _helpers import calc_distance_single_to_single, calc_distance_list_to_single, calc_distance_list_to_list, calculate_cheapest_option_to_final_destination, \
...
def check_if_benchmark_possible(data, configuration, complete_infrastructure):
    ...
    if infrastructure_available:
        ...
        if (reachable_from_start != (None, None)) & (reachable_from_start != (None, None)):
...
def find_shipping_benchmark_solution(data, configuration, all_options, shipping_commodity):
    ...
    conversion_losses = shipping_commodity.get_conversion_loss_of_educt()
    ...
    # calculate the last transportation from ports to
    ...
    # calculate all combinations of costs
    ...
    if shipping_commodity.get_name() not in final_commodities:  # conversion is necessary
        ...
        for c in final_commodities:
            if conversion_options[c]:
                ...
                conversion_costs_c = conversion_costs[c]
                conversion_efficiency = conversion_losses[c]
...
def find_pipeline_shipping_solution(data, configuration, all_options, pipeline_commodity, shipping_commodity):
    ...
    conversion_losses_liquid = shipping_commodity.get_conversion_loss_of_educt()

    conversion_costs_gas = pipeline_commodity.get_conversion_costs()
    conversion_losses_gas = pipeline_commodity.get_conversion_loss_of_educt()

    pipeline_options = all_options[all_options['reachable_from_start']].copy()
    pg_index = [i for i in pipeline_options.index if 'PG' in i]
    pipeline_options = pipeline_options.loc[pg_index, :]

    shipping_options = all_options[all_options['reachable_from_destination']].copy()
    port_index = [i for i in shipping_options.index if 'H' in i]
    shipping_options = shipping_options.loc[port_index, :]
    ...
    if pipeline_commodity.get_transportation_options()['Road']:
        ...
        conversion_costs = conversion_costs_liquid[pipeline_commodity.get_name()]
        conversion_losses = conversion_losses_liquid[pipeline_commodity.get_name()]
    ...
    if distance_to_start <= in_tolerance_distance_option:
        ...
        min_value = (min_value + conversion_costs_liquid[pipeline_commodity.get_name()]) / conversion_losses_liquid[pipeline_commodity.get_name()]
    ...
    if (distance_pipeline_to_port * no_road_multiplier <= max_length_new_segment) & build_new_infrastructure:
        ...
        min_value_before = min_value
        min_value = (min_value + conversion_costs_gas[shipping_commodity.get_name()]) / conversion_losses_gas[shipping_commodity.get_name()]
        costs.append(min_value - min_value_before)
        used_commodities.append(shipping_commodity.get_name())
    ...
    destination_port = shipping_options['distance_to_destination'].idxmin()
    to_destination_distance = shipping_options['distance_to_destination'].min()
    ...
    if used_commodities[-1] not in final_commodities:
        ...
        for c in final_commodities:
            if commodity_object.get_conversion_options_specific_commodity(c):
                ...
                conversion_costs = commodity_object.get_conversion_costs_specific_commodity(c)
                conversion_efficiency = commodity_object.get_conversion_loss_of_educt_specific_commodity(c)
...
def find_pipeline_solution(data, configuration, all_options, pipeline_commodity, road_commodity):
    path_data = '/home/localadmin/Dokumente/Daten_Transportmodell/Daten/'
    ...
    conversion_losses_liquid = road_commodity.get_conversion_loss_of_educt()

    conversion_costs_gas = pipeline_commodity.get_conversion_costs()
    conversion_losses_gas = pipeline_commodity.get_conversion_loss_of_educt()

    pipeline_options_from_start = all_options[all_options['reachable_from_start']].copy()
    pg_index = [i for i in pipeline_options_from_start.index if 'PG' in i]
    pipeline_options_from_start = pipeline_options_from_start.loc[pg_index, :]

    pipeline_options_to_destination = all_options[all_options['reachable_from_destination']].copy()
    pg_index = [i for i in pipeline_options_to_destination.index if 'PG' in i]
    ...
    if g_start_decided == g_destination_decided:
        ...
        if (distance_between_pipelines * no_road_multiplier <= max_length_new_segment) & build_new_infrastructure:
            ...
            # todo: vielleicht eine grenze einführen ab der umgewandelt wird?

            # needs road transportation --> conversion to road commodity
            min_value_before = min_value
            min_value = (min_value + conversion_costs_gas[road_commodity.get_name()]) / conversion_losses_gas[road_commodity.get_name()]
            costs.append(min_value - min_value_before)
            used_commodities.append(road_commodity.get_name())
            ...
            min_value_before = min_value
            min_value = (min_value + conversion_costs_liquid[pipeline_commodity.get_name()]) / conversion_losses_liquid[
                pipeline_commodity.get_name()]
            costs.append(min_value - min_value_before)
            used_commodities.append(pipeline_commodity.get_name())
    ...
    if (distance_pipeline_to_destination * no_road_multiplier <= max_length_new_segment) & build_new_infrastructure:
        ...
        # needs road transportation --> conversion to road commodity
        min_value_before = min_value
        min_value = (min_value + conversion_costs_gas[road_commodity.get_name()]) / conversion_losses_gas[
            road_commodity.get_name()]
        costs.append(min_value - min_value_before)
        used_commodities.append(road_commodity.get_name())
        ...
        for c in [*commodities.keys()]:
            if s_commodity.get_conversion_options_specific_commodity(c):
                if c in final_commodity:
                    ...
                    conversion_costs = s_commodity.get_conversion_costs_specific_commodity(c)
                    conversion_efficiency = s_commodity.get_conversion_loss_of_educt_specific_commodity(c)
