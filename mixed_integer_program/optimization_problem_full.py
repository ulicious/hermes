    ...
    def __init__(self):
        ...
        if True:
            ...
            self.all_nodes_adjusted, self.target_nodes, self.edges, self.production_costs, self.transport_means, \
                self.solution_route, self.cost_route, self.max_costs, self.conversion_edges, self.transport_edges = prepare_data(0, end_location)
            ...
            df.to_csv(path_overall_data + 'conversion_full_edges.csv')

            df = pd.DataFrame(subset_seven).transpose()
            df.to_csv(path_overall_data + 'transport_full_edges.csv')
