...
from _adjust_networks import get_geodata_and_graph_from_network_data_with_intermediate_points
from _calculate_distances_between_infrastructure import calculate_road_distances, get_distances_within_networks
...
def process_seaport_data():
    ...
    for port in features:
        ...
        ports_local.loc[index, 'country'] = port['properties']['country']
        ports_local.loc[index, 'continent'] = port['properties']['continent']

        # add closest point to coastline --> necessary as some ports are not connected to land
        location = Point(ports_local.loc[index, 'longitude'], ports_local.loc[index, 'latitude'])
        new_port_location = nearest_points(coastlines,
                                           location)[0]
        index_closest = coastlines.distance(location).sort_values().index[0]
        closest_point = new_port_location.loc[index_closest].values[0]

        ports_local.loc[index, 'longitude_on_coastline'] = closest_point.x
        ports_local.loc[index, 'latitude_on_coastline'] = closest_point.y

        i += 1

    combinations = list(itertools.combinations(ports_local.index, 2))
    ports_distances = pd.DataFrame(0, index=ports_local.index, columns=ports_local.index.tolist())
    ...
    return ports_local, ports_distances
